...
inline void dialog_handler(ENetPeer* peer, string cch) {
	...
	while (getline(ss, to, '\n')) {
		...
		if (infoDat.size() == 2) {
			...
			if (infoDat[0] == "EnableLegendName" && infoDat[1] == "1") {
				pInfo(peer)->displayName = pInfo(peer)->displayNameBackup + " of Legend";
				return;
			}
			if (infoDat[0] == "EnableLegendName" && infoDat[1] == "0") {
				if (((PlayerInfo*)(peer->data))->adminLevel == 5) {
					...
					((PlayerInfo*)(peer->data))->displayName = "`s@" + ((PlayerInfo*)(peer->data))->tankIDName;
				}
				else if (((PlayerInfo*)(peer->data))->adminLevel == 3) {
					...
					((PlayerInfo*)(peer->data))->displayName = "`w" + ((PlayerInfo*)(peer->data))->tankIDName;
				}
			}

			if (billpicker) {
				if (infoDat[0] == "billitem") billitem = atoi(infoDat[1].c_str());
				if (getItemDef(billitem).properties & Property_Untradable) {
					Send_::console_msg(peer, "`oUntradeable items can't be advertised on billboards.");
					Send_::talk_bubble(peer, ((PlayerInfo*)(peer->data))->netID, "`wUntradeable items can't be advertised on billboards.", 0, false);
					continue;
				}
				if (getItemDef(billitem).blockType == BlockTypes::LOCK) {
					Send_::console_msg(peer, "`oNo no no.");
					Send_::talk_bubble(peer, ((PlayerInfo*)(peer->data))->netID, "`wNo no no.", 0, false);
					continue;
				}
			...
			if (isBillBoard) {
				...
				if (contains_non_int2 == true) {
					Send_::console_msg(peer, "`oPrice can't be Zero. That's free!");
					Send_::talk_bubble(peer, ((PlayerInfo*)(peer->data))->netID, "`wPrice can't be Zero. That's free!", 0, false);
					continue;
				}
				if (billboardprice == "") {
					Send_::console_msg(peer, "`oPrice can't be Zero. That's free!");
				...
				if (wlperitem == "1" && itemperwl == "1") {
					Send_::console_msg(peer, "`oYou can't select both 'locks per item' and 'items per lock'.");
					Send_::talk_bubble(peer, ((PlayerInfo*)(peer->data))->netID, "`wYou can't select both 'locks per item' and 'items per lock'.", 0, false);
					continue;
				}
				if (wlperitem == "0" && itemperwl == "0") {
					Send_::console_msg(peer, "`oYou need to pick one pricing method - 'locks per item' or 'items per lock'.");
					Send_::talk_bubble(peer, ((PlayerInfo*)(peer->data))->netID, "`wYou need to pick one pricing method - 'locks per item' or 'items per lock'.", 0, false);
					continue;
				}
				if (itemperwl == "1") {
			...
			if (infoDat[0] == "EnableDrName" && infoDat[1] == "1") {
				pInfo(peer)->displayName = "`4Dr." + pInfo(peer)->displayNameBackup;
				return;
			}
			if (infoDat[0] == "EnableDrName" && infoDat[1] == "0") {
				if (((PlayerInfo*)(peer->data))->adminLevel == 5) {
					((PlayerInfo*)(peer->data))->displayName = "`6@" + ((PlayerInfo*)(peer->data))->tankIDName;
				}
				else if (((PlayerInfo*)(peer->data))->adminLevel == 4) {
					...
					if (infoDat.at(0) == "count") {
						if (pInfo(peer)->haveGrowId and pInfo(peer)->inGame and pInfo(peer)->passed_data and not pInfo(peer)->tankIDName.empty() and not pInfo(peer)->lobby_guest_acc and pInfo(peer)->currentWorld != "EXIT") {
							...
							if (p != worlds.end()) {
								...
								else if (pInfo(peer)->lasttrash_id == 18 or pInfo(peer)->lasttrash_id == 32) {
	...
	if (btn == "emojis_info")
	{
		std::ifstream news("growmoji.txt");
	...
	else if (btn == "manage_title")
	{
		string Doctor = ""; string Legend = ""; int LG = 0;
		if (((PlayerInfo*)(peer->data))->isOfLegend == true) {
			Legend = "\nadd_checkbox|EnableLegendName| `o'of Legend'|" + to_string(LG) + "|";
		}
		if (pInfo(peer)->isDrTitle = true) {
			Doctor = "\nadd_checkbox|EnableDrName| `o'Dr.'|" + to_string(LG) + "|";
		}
		Send_::dialog_(peer, "set_default_color|`o\nadd_label|big|No Titles Obtained|right|\nadd_spacer|small|" + Legend + Doctor + "\nadd_button|EnableDisableTitle|OK|noflags|0|0|\nend_dialog|title_edit||");
	}
