...
struct PlayerInfo {
	...
	//Player Tutorial
	bool freshmen = false;
	bool completed_tutorial = false;
	bool create_world = false;
	bool open_growpedia = false;
	bool break_blocks = false;
	bool plant_seeds = false;
	bool splice_seeds = false;
	bool build_blocks = false;
	bool wrench_blocks = false;
	bool setup_door = false;
	bool buy_lock = false;
	bool secure_world = false;
	bool secure_world2 = false;
...
inline void Auto_ban(ENetPeer* peer, long long time_, string r_) {
	...
	send_(peer, "action|log\nmsg|`oWarning from `4System`o: You've been `4AUTO-BANNED `ofrom GrowMecha for " + OutputBanTime(calcBanDuration(GetCurrentTimeInternalSeconds() + (time_ * 3600))) + "");
	...
	Send_::notif_(peer, "`0Warning from `4System`0: You've been `4AUTO-BANNED `0from GrowMecha for " + OutputBanTime(calcBanDuration(GetCurrentTimeInternalSeconds() + (time_ * 3600))) + "", "audio/hub_open.wav", "interface/atomic_button.rttex");
...
inline bool isDev(ENetPeer* peer) {
	if (pInfo(peer)->adminLevel == 5) return true;
	else return false;
}
inline bool isSuperMod(ENetPeer* peer) {
	if (pInfo(peer)->adminLevel == 4) return true;
	else return false;
}
inline bool isMod(ENetPeer* peer) {
	if (pInfo(peer)->adminLevel >= 3) return true;
	else return false;
}
inline bool isVipPlus(ENetPeer* peer) {
	if (pInfo(peer)->adminLevel == 2) return true;
	else return false;
}
inline bool isVip(ENetPeer* peer) {
	if (pInfo(peer)->adminLevel == 1) return true;
	else return false;
}
inline void player_update(ENetPeer* peer) {
	if (pInfo(peer)->haveGrowId and pInfo(peer)->rawName != "" and peer->data != NULL and !pInfo(peer)->passed_data) {
		try {
			...
			pInfo(peer)->freshmen = j["freshmen"].get<bool>();
			pInfo(peer)->completed_tutorial = j["completed_tutorial"].get<bool>();
			pInfo(peer)->create_world = j["create_world"].get<bool>();
			pInfo(peer)->open_growpedia = j["open_growpedia"].get<bool>();
			pInfo(peer)->break_blocks = j["break_blocks"].get<bool>();
			pInfo(peer)->plant_seeds = j["plant_seeds"].get<bool>();
			pInfo(peer)->splice_seeds = j["splice_seeds"].get<bool>();
			pInfo(peer)->build_blocks = j["build_blocks"].get<bool>();
			pInfo(peer)->wrench_blocks = j["wrench_blocks"].get<bool>();
			pInfo(peer)->setup_door = j["setup_door"].get<bool>();
			pInfo(peer)->buy_lock = j["buy_lock"].get<bool>();
			pInfo(peer)->secure_world = j["secure_world"].get<bool>();
			pInfo(peer)->secure_world2 = j["secure_world2"].get<bool>();
			pInfo(peer)->send_message = j["send_message"].get<bool>();
...
inline void save_pinfo(ENetPeer* peer) {
	...
	if (pInfo(peer)->haveGrowId and pInfo(peer)->inGame and pInfo(peer)->passed_data and not pInfo(peer)->tankIDName.empty() and not pInfo(peer)->lobby_guest_acc and not pInfo(peer) == NULL) {
		try {
			...
			j["freshmen"] = pInfo(peer)->freshmen;
			j["completed_tutorial"] = pInfo(peer)->completed_tutorial;
			j["create_world"] = pInfo(peer)->create_world;
			j["open_growpedia"] = pInfo(peer)->open_growpedia;
			j["break_blocks"] = pInfo(peer)->break_blocks;
			j["plant_seeds"] = pInfo(peer)->plant_seeds;
			j["splice_seeds"] = pInfo(peer)->splice_seeds;
			j["build_blocks"] = pInfo(peer)->build_blocks;
			j["wrench_blocks"] = pInfo(peer)->wrench_blocks;
			j["setup_door"] = pInfo(peer)->setup_door;
			j["buy_lock"] = pInfo(peer)->buy_lock;
			j["secure_world"] = pInfo(peer)->secure_world;
			j["secure_world2"] = pInfo(peer)->secure_world2;
			j["send_message"] = pInfo(peer)->send_message;
...
inline void player_reg(ENetPeer* peer, string user_, string pass, string pass_v, string email) {
	...
	if (user_.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") != string::npos or pass.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789@#") != string::npos or pass_v.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789@#") != string::npos or email.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789@.") != string::npos) {
		...
		try {
			...
			j["Ances"] = 0;
			j["freshmen"] = true;
			j["completed_tutorial"] = true;
			j["create_world"] = false;
			j["open_growpedia"] = false;
			j["break_blocks"] = false;
			j["plant_seeds"] = false;
			j["splice_seeds"] = false;
			j["build_blocks"] = false;
			j["wrench_blocks"] = false;
			j["setup_door"] = false;
			j["buy_lock"] = false;
			j["secure_world"] = false;
			j["secure_world2"] = false;
			j["send_message"] = false;
...
inline void settingname_(ENetPeer* peer) {
	...
	switch (pInfo(peer)->adminLevel) {
		case 5:
			pInfo(peer)->displayName = "`6@" + pInfo(peer)->displayNameBackup;
			break;
		case 4:
			pInfo(peer)->displayName = "`s@" + pInfo(peer)->displayNameBackup;
			break;
		case 3:
			pInfo(peer)->displayName = "`#@" + pInfo(peer)->displayNameBackup;
			break;
		case 2:
			pInfo(peer)->displayName = "`e@" + pInfo(peer)->displayNameBackup;
			break;
		case 1:
			pInfo(peer)->displayName = "`q@" + pInfo(peer)->displayNameBackup;
			break;
		case 0:
