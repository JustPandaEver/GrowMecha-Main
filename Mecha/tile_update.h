...
inline void tile_update(int x, int y, int tile_, int c_b, ENetPeer* peer) {
	...
	if (p != worlds.end()) {
			...
			case 18: { //fist
				...
				if (block_.foreground == 202 or block_.foreground == 204 or block_.foreground == 206) {
					if (block_.opened and not isDev(peer) and not isSLOwner(peer, world, x, y)) {
						Send_::talk_bubble(peer, pInfo(peer)->netID, block_.string_data + "'s `$" + itemDefs.at(block_.foreground).name + ". `w(`wOpen to public`w) ", 0, true), Send_::OnPlayPositioned(peer, "audio/punch_locked.wav", pInfo(peer)->netID, true);
						return;
					}
					else if (isSLAdmin(peer, world, x, y) and not isDev(peer) and not isSLOwner(peer, world, x, y)) {
						Send_::talk_bubble(peer, pInfo(peer)->netID, block_.string_data + "'s `$" + itemDefs.at(block_.foreground).name + ". `w(`9Access`w)", 0, true), Send_::OnPlayPositioned(peer, "audio/punch_locked.wav", pInfo(peer)->netID, true);
						return;
					}
					else {
						if (not isDev(peer) and not isSLOwner(peer, world, x, y)) {
							Send_::talk_bubble(peer, pInfo(peer)->netID, block_.string_data + "'s `$" + itemDefs.at(block_.foreground).name + ". `w(`4No access`w)", 0, true), Send_::OnPlayPositioned(peer, "audio/punch_locked.wav", pInfo(peer)->netID, true);
							return;
						}
					}
				}
		...
		if (tile_ == 18) {
			...
			if (world->items[x + (y * world->width)].foreground == 11880) {
				if (world->weather == 59) {
					world->weather = 0;
				}
				else {
